...
def print_model_results(
    ...
    for feature in features[:batch_size]:
        all_input_ids.append(feature.input_ids)
        all_input_mask.append(feature.input_mask)
        all_segment_ids.append(feature.segment_ids)
        all_label_ids.append(feature.label_id)
    input_ids = tf.constant(
        all_input_ids, shape=[batch_size, seq_length],
        dtype=tf.int32)
    input_mask = tf.constant(
        all_input_mask, shape=[batch_size, seq_length],
        dtype=tf.int32)
    segment_ids = tf.constant(
        all_segment_ids, shape=[batch_size, seq_length],
        dtype=tf.int32)
    label_ids = tf.constant(
        all_label_ids, shape=[batch_size, seq_length],
        dtype=tf.int32)
    is_training = False

    output_layer = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,
        num_labels, use_one_hot_embeddings, get_output_layer=True)
    ...
    with tf.Session() as sess:
        ...
        a = sess.run(output_layer)
        print(a)
        for x in a:
            print(x)

    import numpy as np
    np.save('test_array.npy', a)
    print(a.shape)
    print(all_input_ids)
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        print_model_results(
            eval_features, seq_length=500, is_training=False, drop_remainder=False,
            bert_config=bert_config,
            num_labels=len(label_list), use_one_hot_embeddings=False)
